...
const STAGE_INFO: Record<string, { label: string; description: string; tasks: string[] }> = {
  customer: {
    label: 'Customer',
  ...
  design: {
    label: 'System design',
    description: 'MCS-compliant system design completed',
    tasks: ['Room-by-room heat loss calculated', 'System design specified', 'Hydraulic diagram produced', 'Design signed off'],
  },
  proposal: {
    label: 'Proposal',
    description: 'Customer proposal and quotation issued',
    tasks: ['Quotation prepared', 'Proposal document sent to customer', 'Customer questions addressed'],
  },
  acceptance: {
  ...
  bus_application: {
    ...
    tasks: ['BUS application submitted to Ofgem', 'Application reference obtained', 'Grant approval confirmed'],
  },
  materials: {
    label: 'Materials',
    description: 'Equipment and materials ordered and delivered',
    tasks: ['Equipment ordered from supplier', 'Delivery date confirmed', 'Materials received and checked'],
  },
  installation: {
    label: 'Installation',
    description: 'Heat pump system installed',
    tasks: ['Installation completed', 'Building regulations notification submitted', 'Commissioning form prepared'],
  },
  commissioning: {
    label: 'Commissioning',
    description: 'System commissioned and tested',
    tasks: ['System commissioned', 'MCS commissioning checklist completed', 'Pressure test carried out', 'Commissioning pack uploaded'],
  },
  handover: {
    label: 'Handover',
    description: 'System handed over to customer',
    tasks: ['Customer handover completed', 'User manual provided', 'Warranty registered', 'MCS certificate issued', 'BUS grant redeemed'],
  },
}
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <a href="/jobs" className="text-xs text-gray-400 hover:text-gray-600 transition-colors">← All jobs</a>
            ...
            <div className="flex items-center gap-3 text-right">
              {customer?.epc_rating && (
                <div className="text-center">
                  ...
                  <div className="text-lg font-semibold text-gray-900">{customer.epc_rating}</div>
                </div>
              )}
              {heatLoss && (
                <div className="text-center border-l border-gray-100 pl-3">
                  <div className="text-xs text-gray-400 mb-0.5">Heat loss</div>
              ...
              <div className="text-center border-l border-gray-100 pl-3">
                ...
                <div className={`text-xs font-medium px-2 py-1 rounded-full ${
                  job?.bus_status === 'eligible' || job?.bus_status === 'approved' || job?.bus_status === 'redeemed'
                    ? 'bg-emerald-50 text-emerald-700'
                    : 'bg-gray-100 text-gray-500'
                }`}>
                  {job?.bus_status === 'eligible' ? '£7,500' :
                   job?.bus_status === 'approved' ? 'Approved' :
                   job?.bus_status === 'redeemed' ? 'Redeemed' : 'Not claiming'}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6">
              ...
              return (
                <div key={stage} className="flex items-center flex-shrink-0">
                  <button
                    ...
                    className={`flex flex-col items-center px-2 py-2 rounded-lg transition-colors min-w-[72px] ${
                  ...
                  {i < STAGE_ORDER.length - 1 && (
                    <div className={`w-4 h-0.5 flex-shrink-0 ${
                      getStageStatus(STAGE_ORDER[i + 1]) !== 'locked' || status === 'complete'
                        ? 'bg-emerald-300'
                        : 'bg-gray-100'
                    }`} />
                  )}
          ...
          {activeStage && (
            <div className={`border rounded-xl p-5 ${
              ...
              getStageStatus(activeStage) === 'complete' ? 'border-gray-200 bg-gray-50' :
              ...
              <div className="flex items-start justify-between mb-4">
                ...
                <div className={`text-xs font-medium px-2.5 py-1 rounded-full flex-shrink-0 ml-4 ${
                  ...
                  getStageStatus(activeStage) === 'flagged' ? 'bg-red-50 text-red-700' :
                  'bg-gray-100 text-gray-400'
                }`}>
                  {getStageStatus(activeStage) === 'complete' ? 'Complete' :
                   getStageStatus(activeStage) === 'in_progress' ? 'In progress' :
                   getStageStatus(activeStage) === 'flagged' ? 'Flagged' : 'Locked'}
                </div>
              </div>
              ...
              {getStageStatus(activeStage) === 'in_progress' && (
                ...
                >
                  {completing === activeStage ? 'Completing...' : `Complete: ${STAGE_INFO[activeStage]?.label} →`}
                </button>
              )}

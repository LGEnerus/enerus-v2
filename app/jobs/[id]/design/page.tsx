...
export type RoomData = {
  ...
  widthMm: number
  heightMm: number
  areaMm2: number          // from polygon if drawn, else length*width
  extWallU: number
  windowU: number
  doorU: number
  floorU: number
  ceilingU: number
  extWallAreaMm2: number   // 0 = auto estimate
  windowAreaMm2: number    // 0 = auto estimate
  doorAreaMm2: number
  floorAdj: string         // ground / heated / unheated / outside
  ceilingAdj: string       // heated / roof / unheated / outside
  achOverride: number | null
  hasOpenFlue: boolean
  fabricLossW: number
  ventLossW: number
  totalLossW: number
  canvasRoomId?: string    // links to canvas polygon
}

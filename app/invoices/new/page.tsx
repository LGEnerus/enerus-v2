...
function NewInvoiceInner() {
  ...
  const [form, setForm] = useState({
    ...
    site_id: '',
    job_id: preJobId,
    invoice_type: 'final' as 'deposit' | 'interim' | 'final' | 'standalone',
    title: '',
    site_address_line1: '',
    site_postcode: '',
    customer_notes: '',
  ...
  async function load() {
    ...
    if (preJobId) {
      // Set invoice_type to standalone if no job, or detect from context
      setForm(p => ({ ...p, invoice_type: 'final' }))
    }

    if (!preCustomerId && !preJobId) {
      setForm(p => ({ ...p, invoice_type: 'standalone' }))
    }
  }
  ...
  function customerName(c: any) { return c.is_company && c.company_name ? c.company_name : `${c.first_name} ${c.last_name}` }
      ...
      <div>
        ...
        <label className="relative block cursor-pointer group">
          ...
          <div className="w-full bg-gray-900 border border-gray-700 rounded-xl px-3 py-2 text-sm flex items-center justify-between group-hover:border-amber-500/60 transition-colors">
            <span className={value ? 'text-gray-100' : 'text-gray-600'}>{value ? new Date(value + 'T00:00:00').toLocaleDateString('en-GB', { day:'numeric', month:'long', year:'numeric' }) : 'Click to set'}</span>
  ...
  async function save() {
    ...
    try {
      ...
      const { data: inv, error: invErr } = await (supabase as any).from('invoices').insert({
        ...
        reference: refData, status: 'draft',
        ...
        total_gross: totalGross, amount_due: totalGross,
        created_by: userId,
      }).select().single()
      ...
      const items = lineItems.filter(i => i.name.trim()).map((i, idx) => ({
        invoice_id: inv.id, sort_order: idx, name: i.name,
    ...
    <div className="min-h-screen bg-gray-950">
      <div className="bg-gray-900 border-b border-gray-800 px-6 h-14 flex items-center gap-3 sticky top-0 z-20">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          <button onClick={save} disabled={saving} className="bg-amber-500 hover:bg-amber-400 disabled:bg-gray-700 text-gray-950 text-xs font-bold px-5 py-2 rounded-lg">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
              <div className="text-sm font-semibold text-white mb-4">Invoice type</div>
              <div className="grid grid-cols-2 gap-2">
                {[
                  { key: 'deposit', label: 'Deposit', desc: 'Upfront payment before work' },
                  { key: 'interim', label: 'Interim', desc: 'Stage payment during job' },
                  { key: 'final', label: 'Final', desc: 'Remaining balance on completion' },
                  { key: 'standalone', label: 'Standalone', desc: 'Not linked to a specific job' },
                ].map(t => (
                  <button key={t.key} onClick={() => upd({ invoice_type: t.key as any })}
                    className={`text-left p-3 rounded-xl border transition-colors ${form.invoice_type === t.key ? 'border-amber-500 bg-amber-500/10' : 'border-gray-700 hover:border-gray-500'}`}>
                    <div className={`text-sm font-medium ${form.invoice_type === t.key ? 'text-amber-300' : 'text-gray-200'}`}>{t.label}</div>
              ...
              {selectedCustomer ? (
                <div className="space-y-3">
                  <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-3 flex items-center justify-between">
                    <div><div className="text-sm font-medium text-amber-300">{customerName(selectedCustomer)}</div><div className="text-xs text-gray-500">{selectedCustomer.email}</div></div>
                  ...
                  {sites.length > 0 && (
                    <div>
                      ...
                      <select value={form.site_id} onChange={e => { const site = sites.find((s: any) => s.id === e.target.value); upd({ site_id: e.target.value, site_address_line1: site?.address_line1 || '', site_postcode: site?.postcode || '' }) }} className={inp}>
                        {sites.map((s: any) => <option key={s.id} value={s.id}>{s.name}{s.postcode ? ` — ${s.postcode}` : ''}</option>)}
                      </select>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="max-h-48 overflow-y-auto space-y-1">
                    {filteredCustomers.map(c => (
                      <button key={c.id} onClick={() => selectCustomer(c)} className="w-full text-left px-3 py-2.5 rounded-xl bg-gray-800 hover:bg-gray-700 transition-colors">
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-4">
              <div className="text-sm font-semibold text-white">Invoice details</div>
              <div><label className={lbl}>Title</label><input className={inp} value={form.title} onChange={e => upd({ title: e.target.value })} placeholder="e.g. Boiler installation — deposit"/></div>
              <div className="grid grid-cols-2 gap-4">
                <DateField label="Invoice date" value={form.invoice_date} onChange={v => upd({ invoice_date: v })}/>
                <DateField label="Due date" value={form.due_date} onChange={v => upd({ due_date: v })}/>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className={lbl}>Site address</label><input className={inp} value={form.site_address_line1} onChange={e => upd({ site_address_line1: e.target.value })}/></div>
                <div><label className={lbl}>Postcode</label><input className={inp} value={form.site_postcode} onChange={e => upd({ site_postcode: e.target.value })}/></div>
              </div>
              <div><label className={lbl}>Notes for customer</label><textarea className={`${inp} resize-none`} rows={2} value={form.customer_notes} onChange={e => upd({ customer_notes: e.target.value })}/></div>
                ...
                {lineItems.map((item, idx) => (
                  <div key={item.id} className="bg-gray-800 rounded-xl p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-5 gap-2">
                      ...
                      <div><div className="text-xs text-gray-600 mb-1">Cost</div><input type="number" min="0" step="0.01" value={item.cost_price} onChange={e => updateItem(item.id, { cost_price: parseFloat(e.target.value) || 0 })} className="w-full bg-gray-700 border border-gray-600 rounded-lg px-2 py-1.5 text-sm text-gray-100 focus:outline-none focus:border-amber-500"/></div>
                      <div><div className="text-xs text-gray-600 mb-1">VAT</div><select value={item.vat_rate} onChange={e => updateItem(item.id, { vat_rate: e.target.value as VatRate })} className="w-full bg-gray-700 border border-gray-600 rounded-lg px-2 py-1.5 text-sm text-gray-100 focus:outline-none focus:border-amber-500"><option value="standard">20%</option><option value="reduced">5%</option><option value="zero">0%</option><option value="exempt">Exempt</option></select></div>
                    </div>
          ...
          <div>
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 sticky top-20 space-y-3">
              ...
              <div className="space-y-2">
                ...
                <div className="border-t border-gray-700 pt-2 flex justify-between"><span className="text-sm font-semibold text-white">Total</span><span className="text-xl font-bold text-amber-400">{formatCurrency(totalGross)}</span></div>
              </div>
              <button onClick={save} disabled={saving} className="w-full bg-amber-500 hover:bg-amber-400 disabled:bg-gray-700 text-gray-950 font-bold py-3 rounded-xl">{saving ? 'Saving…' : 'Create invoice'}</button>

...
type Profile = {
  ...
  employers_liability_expiry: string
  umbrella_agreement_signed: boolean
  status: string
...
export default function ProfilePage() {
  ...
  function calcCompletion(p: Partial<Profile>): number {
    const checks = [
      !!p.company_name,
      !!p.director_name,
      !!p.address_line1,
  ...
  async function handleSave() {
    ...
    const status = pct === 100 ? 'active' : pct > 0 ? 'incomplete' : 'incomplete'

    const payload = { ...profile, completion_pct: pct, status }
  ...
  function update(field: keyof Profile, value: any) {
  ...
  function toggleCompetency(val: string) {
    ...
    if (current.includes(val)) {
      update('competencies', current.filter(c => c !== val))
    } else {
      update('competencies', [...current, val])
    }
  }
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-xs text-gray-400 mt-0.5">
                  {profile.mcs_number || 'MCS number not set'}
                </div>
              </div>
            </div>
        ...
        {activeTab === 'business' && (
          <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Apex Renewables Ltd"
              ...
              <div>
                ...
                <input
                  type="text"
                  value={profile.companies_house_number ?? ''}
                  onChange={e => update('companies_house_number', e.target.value)}
                  placeholder="14782651"
                  className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 transition-colors"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="https://apexrenewables.co.uk"
                  className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 transition-colors"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                value={profile.mcs_number ?? ''}
                onChange={e => update('mcs_number', e.target.value)}
                placeholder="MCS/H/12749"
                className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 transition-colors"
              />
            </div>
        ...
        {activeTab === 'compliance' && (
          <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
            ...
            <div>
              ...
              <input
                type="date"
                value={profile.public_liability_expiry ?? ''}
                onChange={e => update('public_liability_expiry', e.target.value)}
                className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 transition-colors"
              />
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">
                Employer&apos;s liability insurance expiry *
              </label>
              <input
                type="date"
                value={profile.employers_liability_expiry ?? ''}
                onChange={e => update('employers_liability_expiry', e.target.value)}
                className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 transition-colors"
              />
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500 mt-0.5">
                    I confirm I have read and agree to the Enerus Plus MCS Umbrella Scheme terms and conditions, and that all information provided is accurate and up to date.
                  </div>
                </div>
              </div>
            </div>
              ...
              {[
                ...
                <div key={item.label} className="flex items-center gap-2.5">
                  <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 ${
                    item.done ? 'bg-emerald-100' : 'bg-gray-200'
                  }`}>
                    {item.done && (
        ...
        {activeTab === 'documents' && (
          <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
            <p className="text-xs text-gray-500 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2.5">
              Document uploads will be enabled once your business details are saved. Accepted formats: PDF, JPG, PNG (max 10MB each).
            </p>
            ...
            {[
              ...
              <div key={doc.label} className="flex items-center justify-between p-4 border border-gray-200 rounded-xl">
                ...
                <button
                  disabled
                  className="text-xs bg-gray-100 text-gray-400 px-4 py-2 rounded-lg cursor-not-allowed"
                >
                  Upload
                </button>
              </div>
